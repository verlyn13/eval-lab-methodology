...
In plain terms: the frozen module's own documentation claims more than the hash actually proves, so
a matching hash must not be read as proof that two runs are numerically equivalent.
...
matches. A bridge — a recorded human decision that knowingly permits comparing two different
serving stacks (an operator-attested exception) — likewise does not prove equivalence.
